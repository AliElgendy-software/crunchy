...
import {
    ShoppingCart, UtensilsCrossed, Egg, Utensils, Phone, MapPin, Globe, Mail,
    ShoppingBasket, Star, Flame, Clock, Info, ArrowRight, X, Plus, Minus, Trash2,
    Send, ChevronRight, CheckCircle2, Package
} from "lucide-react";
import { Toaster, toast } from "sonner";
import { cn } from "../../lib/utils";
import { MENU_DATA } from "../../data";
import { store } from "../../shared/store";
import { useMenu } from "../../shared/hooks/useStore";
        ...
        <nav className={cn(
            ...
            <div className="flex items-center gap-3">
                <button
                    ...
                    className="hidden sm:flex items-center gap-2 bg-white/5 border border-white/10 px-4 py-2 rounded-full text-[10px] font-black text-white/40 hover:text-white hover:border-primary/40 transition-all uppercase tracking-widest"
                >
                    <ChevronRight className="w-3 h-3" /> Staff Portal
                </button>
    ...
    <section className="relative min-h-screen flex flex-col justify-center items-center px-6 overflow-hidden">
        <div className="absolute inset-0 z-0">
            <img
                ...
                alt="Food" className="w-full h-full object-cover opacity-40 scale-105" referrerPolicy="no-referrer"
            />
        ...
        <div className="relative z-10 text-center max-w-6xl mx-auto pt-20">
            <motion.div initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1 }}>
                <div className="flex justify-center mb-8">
                    <span className="px-6 py-2 rounded-full bg-white/5 backdrop-blur-md border border-white/10 text-primary text-[10px] font-black uppercase tracking-[0.5em]">
                        Crunchy • Dimensional Gastronomy
                    </span>
                </div>
                <h1 className="relative">
                    <span className="block text-[12vw] md:text-[160px] font-black font-headline leading-[0.75] tracking-tighter text-white uppercase italic mix-blend-difference">TASTE THE</span>
                    <span className="block text-[15vw] md:text-[200px] font-black font-headline leading-[0.75] tracking-tighter text-primary uppercase italic text-glow -mt-4 md:-mt-8">CRUNCH</span>
                </h1>
                <div className="mt-12 max-w-2xl mx-auto">
                    ...
                    <div className="flex flex-col sm:flex-row gap-6 justify-center items-center">
                        <a href="#crepes" className="w-full sm:w-auto bg-primary text-white px-14 py-6 rounded-full font-black text-xl shadow-[0_20px_50px_rgba(186,0,39,0.4)] hover:scale-105 active:scale-95 transition-all flex items-center justify-center gap-4 group">
                            اطلب الآن <ArrowRight className="w-6 h-6 group-hover:translate-x-2 transition-transform" />
                        </a>
                        <a href="#crepes" className="w-full sm:w-auto bg-white/5 backdrop-blur-2xl border border-white/10 text-white px-14 py-6 rounded-full font-black text-xl hover:bg-white/10 transition-all">
                            تصفح المنيو
                        </a>
                    </div>
                </div>
...
const CategoryBar = ({ activeCategory, setActiveCategory }: { activeCategory: string; setActiveCategory: (id: string) => void }) => {
    const scrollToCategory = (id: string) => {
        ...
        if (el) window.scrollTo({ top: el.getBoundingClientRect().top - document.body.getBoundingClientRect().top - 100, behavior: "smooth" });
    };
    return (
        <div className="sticky top-0 z-40 bg-black/60 backdrop-blur-2xl border-b border-white/5 py-4">
            <div className="flex items-center gap-3 px-6 overflow-x-auto no-scrollbar">
                {CATEGORIES.map(cat => (
                    <button key={cat.id} onClick={() => scrollToCategory(cat.id)}
                        className={cn("flex items-center gap-2 px-6 py-2.5 rounded-full whitespace-nowrap text-xs font-black uppercase tracking-widest transition-all duration-500",
                            activeCategory === cat.id ? "bg-primary text-white shadow-xl shadow-primary/30 scale-105" : "bg-white/5 text-white/40 hover:bg-white/10 hover:text-white"
                        )}>
                        {cat.icon}{cat.name}
                    </button>
                ))}
...
const SectionHeader = ({ title, subtitle, id }: { title: string; subtitle?: string; id: string }) => (
    <div id={id} className="mb-12 pt-12">
        <div className="flex items-center gap-4 mb-4">
            <div className="h-12 w-2 bg-primary rounded-full shadow-[0_0_20px_rgba(186,0,39,0.5)]" />
...
const PriceBadge = ({ label, price, highlight = false, onClick }: { label: string; price: number | string; highlight?: boolean; onClick?: () => void }) => (
    <button onClick={(e) => { e.stopPropagation(); onClick?.(); }}
        className={cn("flex flex-col items-center justify-center px-5 py-3 rounded-2xl min-w-[85px] transition-all duration-300 active:scale-90",
...
const OrderConfirmed = ({ trackingCode, onClose }: { trackingCode: string; onClose: () => void }) => (
    ...
    >
        <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.8, opacity: 0 }}
            ...
            className="bg-zinc-950 border border-white/10 rounded-3xl p-10 max-w-sm w-full text-center space-y-6"
        >
            ...
            <div>
                ...
                <p className="text-white/40 font-bold text-sm">رمز التتبع الخاص بك</p>
            </div>
            <div className="bg-primary/10 border border-primary/30 rounded-2xl p-4">
                <p className="text-xs font-black text-primary/60 uppercase tracking-widest mb-1">Tracking Code</p>
                <p className="text-3xl font-black text-primary tracking-widest">{trackingCode}</p>
            </div>
            <p className="text-xs text-white/30 font-bold">احفظ هذا الرمز لتتبع طلبك. سيتم تحضير طلبك قريباً.</p>
            <button onClick={onClose} className="w-full bg-primary text-white py-4 rounded-2xl font-black text-sm uppercase tracking-widest hover:bg-primary/90 transition-colors">
                شكراً!
            </button>
        </motion.div>
    </motion.div>
...
export function CustomerApp() {
    ...
    const [cart, setCart] = useState<CartItem[]>(() => {
        try { return JSON.parse(localStorage.getItem("crunchy_cart") || "[]"); } catch { return []; }
    });
    const [isCartOpen, setIsCartOpen] = useState(false);
    const [customerInfo, setCustomerInfo] = useState({ name: "", phone: "", address: "" });
    const [confirmedTracking, setConfirmedTracking] = useState<string | null>(null);

    // ── Live menu from shared store (synced with Admin edits) ──────
    const { menu } = useMenu(5000);
    const getItems = (cat: string): MenuItem[] => menu.filter(m => m.category === cat && m.available);
    ...
    // Persist cart
    useEffect(() => { localStorage.setItem("crunchy_cart", JSON.stringify(cart)); }, [cart]);

    const addToCart = (item: { name: string }, size: string, price: number) => {
        const cartId = `${item.name}-${size}`;
        setCart(prev => {
            const existing = prev.find(i => i.id === cartId);
            if (existing) return prev.map(i => i.id === cartId ? { ...i, quantity: i.quantity + 1 } : i);
            return [...prev, { id: cartId, name: item.name, price, size, quantity: 1, menuItemId: cartId }];
        });
    ...
    const sendOrder = () => {
        if (!customerInfo.name || !customerInfo.phone || !customerInfo.address) {
            toast.error("يرجى إكمال بيانات التوصيل"); return;
        }
        // Save to shared store (kitchen + dashboard will see it)
        ...
        // Also send WhatsApp
        const orderText = cart.map(i => `- ${i.name} (${i.size}) x${i.quantity} = ${i.price * i.quantity} ج.م`).join("\n");
        const msg = `طلب جديد #${order.trackingCode}\n\nالاسم: ${customerInfo.name}\nالهاتف: ${customerInfo.phone}\nالعنوان: ${customerInfo.address}\n\nالطلبات:\n${orderText}\n\nالإجمالي: ${totalPrice} ج.م`;
        window.open(`https://wa.me/201118280661?text=${encodeURIComponent(msg)}`, "_blank");

        setCart([]);
        localStorage.removeItem("crunchy_cart");
        setIsCartOpen(false);
        setConfirmedTracking(order.trackingCode);
    };
    ...
    useEffect(() => {
        const handleScroll = () => {
            ...
            for (let i = CATEGORIES.length - 1; i >= 0; i--) {
                ...
                if (section && section.offsetTop <= scrollPosition) { setActiveCategory(CATEGORIES[i].id); break; }
            }
        };
    ...
    return (
        <div className="min-h-screen bg-black text-white selection:bg-primary selection:text-white font-body" dir="rtl">
            ...
            <div className="relative z-10 bg-black">
                ...
                <div className="px-6 max-w-7xl mx-auto pb-40">
                    ...
                    {/* Crepe Corner */}
                    <section>
                        <SectionHeader id="crepes" title="ركن الكريب" subtitle="Individual Masterpieces (S - M - L - Extra - Roll)" />
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            {getItems('crepeCorner').map((item, i) => (
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            {getItems('mixesCorner').map((item, i) => (
                                <div key={i} className="glass-card p-7 rounded-[2rem] flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6 group hover:bg-white/5 transition-all">
                                    <div className="flex items-center gap-5">
                                        <div className="w-2 h-2 rounded-full bg-primary/40 group-hover:bg-primary transition-colors" />
                                        <span className="text-xl font-black text-white/80 group-hover:text-white transition-colors">{item.name}</span>
                    ...
                    <section className="mt-24">
                        <SectionHeader id="mixes" title="ميكسات ع كيفك" subtitle="The Ultimate Signature Blends" />
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                            {getItems('crepeMixes').map((item, i) => (
                                <motion.div key={i} initial={{ opacity: 0, scale: 0.95 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }}
                                    ...
                                    <div className="flex justify-between items-start">
                                        ...
                                        <div className="bg-primary/20 text-primary p-4 rounded-2xl"><Star className="w-8 h-8 fill-current" /></div>
                                    </div>
                    ...
                    {/* Burgers */}
                    <section className="mt-24">
                        <SectionHeader id="burgers" title="ركن البرجر" subtitle="Flame-Grilled Perfection" />
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
                            <div className="space-y-8">
                                <h4 className="text-2xl font-black text-primary mb-10 flex items-center gap-4 italic uppercase tracking-widest"><Flame className="w-6 h-6" /> Chicken Selection</h4>
                                <div className="grid gap-6">
                                    {getItems('chickenBurgers').map((item, i) => (
                            ...
                            <div className="space-y-8">
                                <h4 className="text-2xl font-black text-primary mb-10 flex items-center gap-4 italic uppercase tracking-widest"><Utensils className="w-6 h-6" /> Beef Selection</h4>
                                <div className="grid gap-6">
                                    {getItems('beefBurgers').map((item, i) => (
                                        <div key={i} className="glass-card p-8 rounded-[2rem] flex justify-between items-center group">
                                            <span className="text-xl font-black text-white/90">{item.name}</span>
                                            <div className="flex gap-4">
                                                <PriceBadge label="Single" price={item.prices.single} onClick={() => addToCart(item, "Single", item.prices.single)} />
                    ...
                    {/* Pizza */}
                    <section className="mt-24">
                        <SectionHeader id="pizza" title="ركن البيتزا" subtitle="Authentic Italian Heritage (S - M - L - XL)" />
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                            {getItems('pizza').map((item, i) => (
                    ...
                    {/* Negresco */}
                    <section className="mt-24">
                        <SectionHeader id="negresco" title="ركن النجرسكو" subtitle="Creamy Pasta Masterpieces" />
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            {getItems('negresco').map((item, i) => (
                    ...
                    {/* Extras */}
                    <section className="mt-24" id="extras">
                        <SectionHeader id="extras-header" title="إضافات" subtitle="Extra Combos & Sides" />
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                            <div className="glass-card p-8 rounded-[2rem] space-y-4">
                                <h4 className="font-black text-primary uppercase tracking-widest text-sm">بيتزا كريب</h4>
                                <div className="flex gap-3">
                                    <PriceBadge label="S" price={MENU_DATA.extras.pizzaCrepe.s} onClick={() => addToCart({ name: "بيتزا كريب" }, "S", MENU_DATA.extras.pizzaCrepe.s)} />
                                    <PriceBadge label="L" price={MENU_DATA.extras.pizzaCrepe.l} highlight onClick={() => addToCart({ name: "بيتزا كريب" }, "L", MENU_DATA.extras.pizzaCrepe.l)} />
                                </div>
                            </div>
                            <div className="glass-card p-8 rounded-[2rem] space-y-4">
                                <h4 className="font-black text-primary uppercase tracking-widest text-sm">كريب كونو</h4>
                                <div className="flex gap-2 flex-wrap">
                                    {MENU_DATA.extras.crepeKono.map((p, i) => (
                                        <PriceBadge key={i} label={['S', 'M', 'L'][i]} price={p} onClick={() => addToCart({ name: "كريب كونو" }, ['S', 'M', 'L'][i], p)} />
                                    ))}
                                </div>
                            </div>
                            <div className="glass-card p-8 rounded-[2rem] space-y-4">
                                <h4 className="font-black text-primary uppercase tracking-widest text-sm">باكيت بطاطس</h4>
                                <div className="flex gap-3">
                                    {MENU_DATA.extras.potatoPacket.map((p, i) => (
                                        <PriceBadge key={i} label={['S', 'L'][i]} price={p} onClick={() => addToCart({ name: "باكيت بطاطس" }, ['S', 'L'][i], p)} />
                                    ))}
                                </div>
                            </div>
                        </div>
                ...
                <div className="max-w-6xl mx-auto">
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-16 mb-24">
                        <div className="space-y-8">
                            ...
                            <p className="text-white/40 font-bold leading-relaxed">نحن نؤمن بأن الطعام هو لغة السعادة، ولذلك نقدم لك أفضل المكونات مع لمسة إبداعية تجعل من كل وجبة ذكرى لا تنسى.</p>
                            <div className="flex gap-4">
                                <div className="w-10 h-10 rounded-full bg-white/5 flex items-center justify-center hover:bg-primary transition-colors cursor-pointer"><Globe className="w-5 h-5" /></div>
                                <div className="w-10 h-10 rounded-full bg-white/5 flex items-center justify-center hover:bg-primary transition-colors cursor-pointer"><Mail className="w-5 h-5" /></div>
                            </div>
                        </div>
                        <div className="space-y-8">
                            <h5 className="text-xs font-black uppercase tracking-[0.3em] text-primary">Contact Us</h5>
                            <div className="space-y-4">
                                {[{ number: "01118280661", label: "للطلب والاستفسار" }, { number: "01026500505", label: "الشكاوي والاقتراحات" }, { number: "01272616964", label: "للطلب والاستفسار" }].map((item, i) => (
                                    <a key={i} href={`tel:${item.number}`} className="flex items-center gap-4 text-white/60 hover:text-white transition-colors group">
                                        <div className="w-8 h-8 rounded-lg bg-white/5 flex items-center justify-center group-hover:bg-primary/20 transition-colors"><Phone className="w-4 h-4 text-primary" /></div>
                        ...
                        <div className="space-y-8">
                            ...
                            <div className="flex items-start gap-4 text-white/60">
                                <div className="w-8 h-8 rounded-lg bg-white/5 flex items-center justify-center shrink-0"><MapPin className="w-4 h-4 text-primary" /></div>
                                <p className="font-black text-lg leading-snug">العزيزية - أمام الكوبري الجديد</p>
                            </div>
                            <button onClick={() => { window.location.hash = '/login'; }}
                                className="flex items-center gap-2 text-xs font-black text-white/30 hover:text-primary transition-colors uppercase tracking-widest mt-8">
                                <Package className="w-4 h-4" /> Staff Portal →
                            </button>
                        </div>
                    </div>
                    <div className="pt-12 border-t border-white/5 flex flex-col md:flex-row justify-between items-center gap-6">
                        <p className="text-sm font-black text-primary/60 uppercase tracking-[0.2em] italic">Developed by Ali Elgendy</p>
            ...
            <button onClick={() => setIsCartOpen(true)}
                ...
                {totalItems > 0 && (
                    <div className="absolute -top-1 -right-1 bg-white text-primary text-[10px] font-black w-5 h-5 rounded-full flex items-center justify-center border-2 border-black">{totalItems}</div>
                )}
            </button>
            ...
            <AnimatePresence>
                {isCartOpen && (
                    <>
                        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                            onClick={() => setIsCartOpen(false)} className="fixed inset-0 bg-black/60 backdrop-blur-md z-[60]" />
                        <motion.div initial={{ x: "-100%" }} animate={{ x: 0 }} exit={{ x: "-100%" }}
                            ...
                            <div className="p-6 border-b border-white/5 flex justify-between items-center">
                                ...
                                <button onClick={() => setIsCartOpen(false)} className="p-2 hover:bg-white/5 rounded-full transition-colors"><X className="w-6 h-6" /></button>
                            </div>
                                    ...
                                    <>
                                            ...
                                            {cart.map(item => (
                                                <div key={item.id} className="glass-card p-4 rounded-2xl flex justify-between items-center gap-4">
                                                    ...
                                                    <div className="flex items-center gap-3 bg-white/5 p-1 rounded-xl">
                                                        <button onClick={() => updateQuantity(item.id, -1)} className="p-1.5 hover:bg-white/10 rounded-lg transition-colors"><Minus className="w-4 h-4" /></button>
                                                        <span className="font-black w-4 text-center">{item.quantity}</span>
                                                        <button onClick={() => updateQuantity(item.id, 1)} className="p-1.5 hover:bg-white/10 rounded-lg transition-colors text-primary"><Plus className="w-4 h-4" /></button>
                                                    </div>
                                                    <button onClick={() => updateQuantity(item.id, -item.quantity)} className="p-2 text-white/20 hover:text-primary transition-colors"><Trash2 className="w-4 h-4" /></button>
                                                </div>
                                            ))}
                                        </div>
                                        <div className="pt-6 border-t border-white/5 space-y-4">
                                            ...
                                            <div className="space-y-3">
                                                <input type="text" placeholder="الاسم بالكامل" value={customerInfo.name}
                                                ...
                                                <input type="tel" placeholder="رقم الهاتف" value={customerInfo.phone}
                                                    onChange={e => setCustomerInfo(p => ({ ...p, phone: e.target.value }))}
                                                    className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-primary/50 transition-colors" />
                                                <textarea placeholder="عنوان التوصيل بالتفصيل" value={customerInfo.address}
                            ...
                            {cart.length > 0 && (
                                <div className="p-6 border-t border-white/5 bg-black/40 backdrop-blur-xl">
                                    ...
                                    <button onClick={sendOrder}
                                        ...
                                        تأكيد الطلب عبر واتساب <Send className="w-5 h-5" />
                                    </button>
                                </div>
                            )}
                        </motion.div>
            ...
            <AnimatePresence>
                {confirmedTracking && <OrderConfirmed trackingCode={confirmedTracking} onClose={() => setConfirmedTracking(null)} />}
            </AnimatePresence>
        </div>
    );
}
